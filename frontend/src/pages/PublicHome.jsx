...
export default function PublicHome() {
  ...
  const handleDocClick = (docId) => {
    ...
    if (token) {
      ...
      navigate('/login', { state: { redirectTo: `/document/${docId}` } });
    }
  };
  ...
  const DocumentCard = ({ doc, rank, showRank = false }) => (
    <div
      ...
      className="relative flex items-start gap-3 p-4 bg-white rounded-lg border border-gray-200 hover:border-gray-300 hover:shadow-sm transition-all cursor-pointer group"
    >
      {/* 价格标签 */}
          ...
          <form onSubmit={handleSearch} className="max-w-2xl mx-auto">
            <div className="relative">
              <input
                ...
                className="w-full px-5 py-4 pl-12 text-base border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-black focus:border-transparent shadow-sm"
              />
              <svg
              ...
              <button
                ...
                className="absolute right-2 top-1/2 -translate-y-1/2 px-5 py-2 bg-black text-white text-sm font-medium rounded-lg hover:bg-gray-800 transition-colors disabled:opacity-50"
              >
                {isSearching ? '搜索中...' : '搜索'}
              </button>
            </div>
        ...
        <div className="max-w-6xl mx-auto px-6 py-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {searchResults.map((doc, index) => (
              <DocumentCard key={doc._id} doc={doc} rank={index + 1} />
            ))}
          </div>
        </div>
      )}
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        {loading ? (
          ...
          <div className="space-y-8">
            {/* Popular Documents - Full Width Cards */}
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {popularDocs.slice(0, 6).map((doc, index) => (
                  <DocumentCard key={doc._id} doc={doc} rank={index + 1} showRank />
                ))}
              </div>
